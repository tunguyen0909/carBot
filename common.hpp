...
cv::Mat preprocess_img(cv::Mat& img) {
    cv::Mat out;
    cv::resize(img, out, cv::Size(Yolo::INPUT_W, Yolo::INPUT_H), cv::INTER_LINEAR);
    return out;
}
...
cv::Rect get_rect(cv::Mat& img, float bbox[4]) {
    ...
    l = std::max(int(bbox[0] - bbox[2]/2.f), 1);
    r = std::min(int(bbox[0] + bbox[2]/2.f), Yolo::INPUT_W-2);
    t = std::max(int(bbox[1] - bbox[3]/2.f), 1);
    b = std::min(int(bbox[1] + bbox[3]/2.f), Yolo::INPUT_H-2);
